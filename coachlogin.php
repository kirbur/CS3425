<?php
 session_start();
    if (isset($_SESSION["loggedin"]) ) {
      header("LOCATION:https://classdb.it.mtu.edu/~krburns/finalproject/coachHome.php");
       return;
    }
    ...
    try {
       ...
       foreach( $dbh->query("select email, password, camp_number, camp.name from coach join camp where coach.camp_number = camp.num;") as $row){
         if(isset($_POST["username"])){
            if($_POST["username"] == $row[0] && $_POST["password"] == $row[1] && $_POST["opt"] == $row[3]){
               ...
               // return;             
                  ...
                  if(!($_POST["opt"] == $row[3])){
                     echo '<script type="text/javascript">alert("You dont work at this camp.");</script>';
                  }
                  echo '<script type="text/javascript">alert("Wrong username or password.");</script>';
                  header("LOCATION:https://classdb.it.mtu.edu/~krburns/finalproject/coachlogin.php");
            }
         }
       }
       $dbh->commit();
