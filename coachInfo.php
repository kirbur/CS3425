<?php session_start(); ?>
<!DOCTYPE HTML>
<html>
<head>
<link rel="stylesheet" type="text/css" href="style.css"/>
<title>Coach Info</title>
</head>
<body>
<div id="wrap">
  <div id="main">
  <h3>Coach Info</h3>
   <div class="scroll" id="coachdiv">
        <?php
           if (isset($_SESSION["loggedin"]) ) {
           } else {
             header("LOCATION:https://classdb.it.mtu.edu/~krburns/finalproject/home.html");
           }
           try {
              $config = parse_ini_file("db.ini");
              $dbh = new PDO($config['dsn'], $config['username'], $config['password']);

              $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $dbh->beginTransaction();
              foreach( $dbh->query("select name, email, price, bio from coach;") as $row){
       	          echo "<div class='ci' >";
                   echo "<h2>Name: ".$row[0]."</h2>";
                   echo "<h2>Email: ".$row[1]."</h2>";
                   echo "<h2>Session Fee: ".$row[2]."</h2>";
                   echo "<h2>Bio:</h2>";
                   echo "<p>".$row[3]."</p>";
                   echo "</div>";
                   echo "</br>";
              }
              $dbh->commit();   
           } catch (PDOException $e) {
             $dbh->rollBack();
             print "Error!" . $e->getMessage()."<br/>";
             die();
           }
        ?>
        
      </div>
   
  </div><!--main-->
  <div id="nav">
  <ul>
      <li>
            <?php 
               $usr = $_SESSION["username"];
               
               try {
                  $config = parse_ini_file("db.ini");
                  $dbh = new PDO($config['dsn'], $config['username'], $config['password']);
                  $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                  $dbh->beginTransaction();
                  $camp = 'camp name';
                  if(isset($_SESSION['camp'])){
                     foreach( $dbh->query("select name from camp where num = ".$_SESSION['camp'].";") as $row){
                       $camp = $row[0];
                     }
                  }
                  $dbh->commit();
               } catch (PDOException $e) {
                 $dbh->rollBack();
                 print "Error!" . $e->getMessage()."<br/>";
                 die();
               }
                              
               echo "<p>".$camp."</p>"; 
               echo "<p>".$usr."</p>"; 
            ?>
      </li>
  		<li><a href="camperHome.php" target="_self">Home</a></li>
      <li><a href="coachInfo.php" target="_self">Coach Info</a></li>
      <li><a href="camperregistration.php" target="_self">Change Schedule</a></li>
      <li><a href="updateCamper.php" target="_self">Change Info</a></li>
      <li><a href="logout.php" target="_self">Logout</a></li>
  	</ul>
  </div><!--nav-->
</div><!--wrap-->
</body>
</html>